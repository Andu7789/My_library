...
This app is a full PWA: it ships a [`manifest.json`](manifest.json) and a
[`service-worker.js`](service-worker.js) that caches the app shell (HTML/CSS/JS
and icons) for offline loading and installability. Supabase sync requests are
never cached and always go straight to the network.

**Desktop (Chrome/Edge)**:
1. Open the app
2. Click the install icon (⊕) in the address bar, or the browser menu → "Install My Reading Library"
...
Once installed, the app opens in its own standalone window (no browser
chrome) and the last-loaded version keeps working without a network
connection — new books/edits still require connectivity to sync via
Supabase.

Icons live in [`icons/`](icons/) and are regenerated with
`python scripts/gen-icons.py` (requires Pillow: `pip install pillow`).
...
- `icons/` - App icons for install/home-screen (generated by `scripts/gen-icons.py`)
